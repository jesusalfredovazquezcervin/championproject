  		...
  		<g:each in="${proveedores?.proveedores}" var="proveedorInstance" status="i">
  			<g:if test="${proveedorInstance}">
  				<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
		  			...
		  			<td><span id="modalProveedores" class='ui-icon ui-icon-check' onclick="${remoteFunction(controller:'clasificacionProd',action:'getProveedor',
						   params:['provid':proveedorInstance.id], update:[success:'prov-seleccionado'], onSuccess='obtenerPartidas()')}"></span></td>
	  			</tr>
  			</g:if>
  		</g:each>
</table>
